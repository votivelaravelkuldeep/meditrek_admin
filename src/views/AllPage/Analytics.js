 ...
 * AdminAnalytics.jsx
 *
 * Super-admin version of the Analytics page.
 * All data is static/mock - no API dependencies.
 */
...
// Mock patient data
const MOCK_PATIENTS = [
  { id: 1, name: "John Smith", age: 45, gender: "Male", diseases: [{ name: "Hypertension" }, { name: "Type 2 Diabetes" }], medications: [{ name: "Lisinopril" }, { name: "Metformin" }], symptoms: "headache, fatigue" },
  { id: 2, name: "Sarah Johnson", age: 32, gender: "Female", diseases: [{ name: "Migraine" }], medications: [{ name: "Sumatriptan" }], symptoms: "severe headache, nausea" },
  { id: 3, name: "Michael Brown", age: 58, gender: "Male", diseases: [{ name: "COPD" }, { name: "Hypertension" }], medications: [{ name: "Albuterol" }, { name: "Lisinopril" }], symptoms: "shortness of breath, wheezing" },
  { id: 4, name: "Emily Davis", age: 27, gender: "Female", diseases: [{ name: "Anxiety" }], medications: [{ name: "Sertraline" }], symptoms: "nervousness, insomnia" },
  { id: 5, name: "David Wilson", age: 62, gender: "Male", diseases: [{ name: "Osteoarthritis" }, { name: "Hypertension" }], medications: [{ name: "Ibuprofen" }, { name: "Amlodipine" }], symptoms: "joint pain, stiffness" },
  { id: 6, name: "Lisa Anderson", age: 41, gender: "Female", diseases: [{ name: "Asthma" }], medications: [{ name: "Fluticasone" }], symptoms: "coughing, chest tightness" },
  { id: 7, name: "Robert Taylor", age: 73, gender: "Male", diseases: [{ name: "Heart Failure" }, { name: "COPD" }], medications: [{ name: "Furosemide" }, { name: "Spiriva" }], symptoms: "fatigue, edema, shortness of breath" },
  { id: 8, name: "Jennifer Martinez", age: 35, gender: "Female", diseases: [{ name: "Depression" }], medications: [{ name: "Escitalopram" }], symptoms: "low mood, loss of interest" },
  { id: 9, name: "William Garcia", age: 51, gender: "Male", diseases: [{ name: "Type 2 Diabetes" }, { name: "Hyperlipidemia" }], medications: [{ name: "Metformin" }, { name: "Atorvastatin" }], symptoms: "frequent urination, thirst" },
  { id: 10, name: "Maria Rodriguez", age: 29, gender: "Female", diseases: [{ name: "Migraine" }], medications: [{ name: "Rizatriptan" }], symptoms: "aura, throbbing headache" },
];
...
const S = {
  ...
  navTitle: { fontSize: 15, fontWeight: 800, color: "#1a202c", letterSpacing: -0.3 },
  ...
  adminBadge: { background: ACCENT, color: "#fff", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 6, letterSpacing: 0.5, textTransform: "uppercase" },
  ...
  th: { padding: "10px 13px", fontSize: 11, fontWeight: 700, color: "#6b7280", background: "#f8f9fc", borderBottom: "1px solid #eaecf2", whiteSpace: "nowrap", letterSpacing: 0.3 },
    ...
    <div style={S.tWrap}>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            {cols.map(c => (
              <th key={c.key} style={{ ...S.th, textAlign: c.align || "left", cursor: c.sortable ? "pointer" : "default" }}
...
function Demographics() {
  ...
  const filteredData = useMemo(() => {
    return MOCK_DEMOGRAPHICS.filter(item => {
  ...
  const crossData = useMemo(() => {
    ...
    const map = {};
    Object.keys(AGE_GROUPS).forEach(ag => {
      map[ag] = { age: ag, Male: 0, Female: 0, Other: 0, "Not Specified": 0, total: 0 };
    });
    ...
    return Object.values(map).map(row => {
      const newRow = { ...row };
      genders.forEach(g => { newRow[g + "_pct"] = pct(row[g], total); });
...
/* ============================================================
   2. DISEASE / DEMOGRAPHICS (simplified with static data)
   ============================================================ */
function DiseaseDemo({ diseases }) {
  ...
  const filteredPatients = useMemo(() => {
    ...
    if (selDiseases.length > 0) {
      patients = patients.filter(p =>
        p.diseases.some(d => selDiseases.includes(d.name))
      );
    }
    if (ageGroup !== "All") {
      patients = patients.filter(p => {
        ...
        if (ageGroup === "19-30") return age >= 19 && age <= 30;
        if (ageGroup === "31-45") return age >= 31 && age <= 45;
        if (ageGroup === "46+") return age >= 46;
        return true;
      });
    }
    if (gender !== "All") {
  ...
  const ageDist = useMemo(() => {
    ...
    filteredPatients.forEach(p => {
      const g = Object.keys(AGE_GROUPS).find(k => AGE_GROUPS[k](p.age));
      if (g) map[g]++;
    });
  ...
  const genderDist = useMemo(() => {
    ...
    filteredPatients.forEach(p => {
      if (base[p.gender] !== undefined) base[p.gender]++;
      else base["Not Specified"]++;
    });
    return Object.entries(base).map(([gender, count]) => ({ gender, count, percentage: pct(count, filteredPatients.length) }));
  }, [filteredPatients]);

  const patientsForTable = filteredPatients.map(p => ({
    name: p.name,
    age: p.age,
    gender: p.gender,
...
/* ============================================================
   3. DISEASE / MEDICATION (simplified)
   ============================================================ */
function DiseaseMedication({ diseases }) {
  ...
  const topDrug = medicationStats.length > 0 ? medicationStats.reduce((a, b) => a.patient_count > b.patient_count ? a : b).medicine_name : "—";
...
/* ============================================================
   4. MEDICATION / DEMOGRAPHICS (simplified)
   ============================================================ */
function MedicationDemo({ medicines }) {
  ...
  const filteredPatients = useMemo(() => {
    ...
    if (selMeds.length > 0) {
      patients = patients.filter(p =>
        p.medications.some(m => selMeds.includes(m.name))
      );
    }
    if (ageGroup !== "All") {
      patients = patients.filter(p => {
  ...
  const medicationDist = useMemo(() => {
    ...
    filteredPatients.forEach(p => {
      p.medications.forEach(m => {
        if (selMeds.length === 0 || selMeds.includes(m.name)) {
          map[m.name] = (map[m.name] || 0) + 1;
        }
      });
    });
    ...
    <div>
      ...
      <div style={S.card}>
        ...
        <DataTable cols={[
          ...
          { key: "conditions", label: "Diseases", sortable: true, render: r => <DChips arr={r.conditions} /> },
          { key: "meds", label: "Medications", sortable: true, render: r => <MChips arr={r.meds} /> },
        ]} rows={patientsForTable} empty="No patients match the selected filters" />
      </div>
    </div>
  );
}
...
/* ============================================================
   5. MEDICATION / DISEASE (simplified)
   ============================================================ */
function MedicationDisease({ medicines }) {
  ...
  const topDisease = diseaseDist.length > 0 ? diseaseDist.reduce((a, b) => a.patient_count > b.patient_count ? a : b).disease : "—";
...
/* ============================================================
   6. MEDICATION / REPORTED HEALTH (simplified)
   ============================================================ */
  ...
  const medHealthData = useMemo(() => {
    ...
    patients.forEach(p => {
      p.medications.forEach(med => {
        if (!medMap.has(med.name)) {
          medMap.set(med.name, { total_patients: 0, symptomMap: new Map() });
        }
        const entry = medMap.get(med.name);
        entry.total_patients++;

        const symptoms = p.symptoms ? p.symptoms.split(", ") : [];
        symptoms.forEach(symptom => {
          entry.symptomMap.set(symptom, (entry.symptomMap.get(symptom) || 0) + 1);
        });
      });
    });
    ...
    for (const [medName, data] of medMap) {
      ...
      for (const [symptom, count] of data.symptomMap) {
        symptoms.push({ symptom, count, percentage: ((count / data.total_patients) * 100).toFixed(1) });
      }
      ...
      result.push({
        ...
        symptoms,
        percentage: ((data.total_patients / MOCK_PATIENTS.length) * 100).toFixed(1),
      });
    }
    return result;
  }, [selMeds]);
...
function CustomizeTable({ diseases, medicines, symptoms }) {
  ...
  const filteredPatients = useMemo(() => {
    let patients = [...MOCK_PATIENTS];
    if (filterDis.length > 0) {
      patients = patients.filter(p => p.diseases.some(d => filterDis.includes(d.name)));
    }
    if (filterMed.length > 0) {
      patients = patients.filter(p => p.medications.some(m => filterMed.includes(m.name)));
    }
    if (filterSymptoms.length > 0) {
      patients = patients.filter(p => {
        const symptomsList = p.symptoms ? p.symptoms.split(", ") : [];
        return filterSymptoms.some(s => symptomsList.includes(s));
      });
    }
    if (ageGroup !== "All") {
      patients = patients.filter(p => {
  ...
  const patientsForTable = filteredPatients.map(p => ({
    ...
    conditions: p.diseases.map(d => d.name),
    meds: p.medications.map(m => m.name),
    reportedHealth: p.symptoms ? p.symptoms.split(", ") : [],
  }));
  ...
  return (
    <div>
      <div style={S.pageHead}><h2 style={S.pageTitle}>Customize Table</h2><p style={S.pageSub}>Build your own lookup — choose columns and stack filters to find exactly who you need</p></div>
      <div style={S.filterBar}>
      ...
      <div style={S.statRow}>
        <StatCard label="Matching Patients" value={filteredPatients.length} sub={`${pct(filteredPatients.length, MOCK_PATIENTS.length)}% of total patients`} />
        <StatCard label="Total Patients" value={MOCK_PATIENTS.length} />
      ...
      <div style={S.card}>
        {cols.length === 0 ? <div style={S.noData}>Select at least one column to display</div> : (
          <DataTable cols={cols} rows={patientsForTable} empty="No patients match the selected filters" />
        )}
      </div>
    </div>
  );

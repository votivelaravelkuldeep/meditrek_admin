...
function Reply() {
  ...
  return (
    <>
      {/* <Typography
        style={{ marginTop: '0px', marginBottom: '12px', display: 'flex', justifyContent: 'flex-end', fontSize: '13px', fontWeight: '500' }}
        variant="h4"
        gutterBottom
      >
      ...
      >
        <Card.Body className='className="custom-modal"'>
          <Formik initialValues={{ subject: '', message: '' }} validationSchema={validationSchema} onSubmit={handleSubmitReply}>
            {({ handleSubmit, errors, touched, isSubmitting }) => (
              <FormikForm noValidate onSubmit={handleSubmit}>
                <Form.Group className="mb-3" controlId="formSubject">
                  <Form.Label style={{ fontSize: '13px', fontWeight: 500 }}>Subject</Form.Label>
                  <Field name="subject" type="text" style={{ fontSize: '13px' }} className={`custom-input custom-search form-control${errors.subject && touched.subject ? ' is-invalid' : ''}`} />
                ...
                <Form.Group className="mb-3" controlId="formMessage">
                  ...
                  <Field
                    as="textarea"
                    name="message"
                    className={`custom-input custom-search form-control${errors.message && touched.message ? ' is-invalid' : ''}`}
                    rows="4"
